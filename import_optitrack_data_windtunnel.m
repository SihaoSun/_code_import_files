function [OT] = import_optitrack_data(name,name_calib,body_names,filt_OT)
...
if filt_OT == 0
    ...
    openedFile = fopen(['Take_filt ',name '.csv'],'r');
end
...
if filt_OT == 0
    ...
    data.CSVdata=csvread(['Take_filt ' name '.csv'],7,0);
end
...
%     data.CSVdata(nanframes,index(nameMatchPartial==1))=NaN;
    ...
    posCO_G0  = interpNan(data.body(n).pos);
    if filt_OT == false
          ...
          quat.qw = qw;
          quat.TIME = data.time;
          pos = bebop_prefilter(pos);
          quat = bebop_prefilter(quat);
          
          posCO_G = [pos.X pos.Y pos.Z];
          qx = quat.qx;
          qy = quat.qy;
          qz = quat.qz;
          qw = quat.qw;
