function [OB] = import_onboard_data_windtunnel(name,string)
if nargin == 0
%     take = select_take(20);
    do_filtering = true;
...
% load on-board data
logdata = csvread(['onboard_log ' name '.csv'],1,0);

% Retrieving labels of the logged data from first line
openedFile = fopen(['onboard_log ' name '.csv'],'r');
...
OB.PosNED = [POS.x POS.y POS.z];
OB.VelNED = [SPEED.x SPEED.y SPEED.z];
